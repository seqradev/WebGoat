...
class OpenRedirectLessonUITest : PlaywrightTest() {
    private lateinit var lessonPage: OpenRedirectLessonPage

    @BeforeEach
    fun navigateToLesson(browser: Browser) {
        val lessonName = LessonName("OpenRedirect")
        val page = Authentication.sylvester(browser)

        lessonPage = OpenRedirectLessonPage(page)
    ...
    fun shouldSolveOpenRedirectLesson() {
        // Task 1
